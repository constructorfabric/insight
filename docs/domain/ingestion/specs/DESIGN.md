...
- Service access via edge routes (PR #224): the umbrella chart configures edge routes (originally `ingress-nginx` Ingresses, now Gateway API HTTPRoutes) for Frontend, API Gateway, Airbyte UI and Argo UI; on a local Kubernetes cluster the Kind config maps host ports 80/443 to the edge proxy. Direct port-forwards (`kubectl port-forward`) remain available for debugging.
