...
| `cpt-presentation-fr-preview-envs` | Path-based `/exp/<name>` on one host, one shared read-only backend, FE-only variation. Per-experiment bundle (`Deployment` + `Service` + one prefix-strip `HTTPRoute`) shipped as the `insight-preview` chart at `deploy/preview/`, applied by hand (#1971). Experiments are a capability gated off on production by default (`cpt-presentation-constraint-experiments-off-prod`, #1973). Shipped |
...
- Deployment: one route object per experiment applied by hand (`Deployment` + `Service` + one routing object with prefix-strip rewrite). Each route attaches to the shared Gateway independently via `parentRef`, so `apply` adds a path and `delete` removes it; no central config is rewritten. The route is a Gateway API `HTTPRoute`: a `PathPrefix` match on `/exp/<name>` plus a native `URLRewrite` filter (`ReplacePrefixMatch: /`) strips the prefix -- no controller-specific annotations. Shipped (#1971) as the `insight-preview` Helm chart at `deploy/preview/`: each experiment is one release named `preview-<name>`, and the experiment slug is validated as a DNS-1123 label of at most 55 characters at template time. Render-contract tests plus a `.github/workflows/preview-helm.yml` lane guard it. The auth return path above shipped (#1972).
