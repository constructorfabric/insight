...
| `cpt-insightspec-adr-cdk-prebuilt-images` | CDK connectors use pre-built ghcr.io images; reconcile derives `dockerRepository`/`dockerImageTag` from descriptor + `IMAGE_REGISTRY` required env; never builds at runtime | §3.2 reconcile-engine, §3.9 Reconciliation Model |
...
**CDK connector lifecycle** (per ADR-0011): Insight CDK connectors are pre-built Docker images published to `ghcr.io/cyberfabric/source-${connector}-insight`. The image tag equals `descriptor.yaml.version`. Reconcile **never builds Docker images at runtime** — it only registers/updates the source_definition with `dockerRepository` and `dockerImageTag` derived from the descriptor + `IMAGE_REGISTRY` required env. First registration uses `source_definitions/create_custom`; subsequent version bumps use `source_definitions/update` (existing `ab_set_definition_image_tag`). Local development image build remains in `lib/cdk-build.sh:cdk_build` (operator-invoked, not part of the reconcile loop).
