...
| `cpt-insightspec-actor-ci-pipeline` | Runs `reconcile-connectors.sh adopt && reconcile-connectors.sh` via the in-cluster reconcile CronWorkflow on a schedule (and on demand from the host) |
