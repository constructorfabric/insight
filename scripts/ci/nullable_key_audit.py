...
def inherits_key(text: str, col: str) -> bool:
    """True when the model passes the key through from an upstream relation
    rather than projecting it with `AS <col>` — `SELECT *`, a `union_by_tag`
    union, a bare `<col>,` select, or a qualified `<alias>.<col>,`. In every
    such case the key's nullability is determined (and audited) upstream, so the
    absence of a local `AS <col>` projection is expected, not a gap.
    """
    if re.search(r"union_by_tag|SELECT\s+\*", text, re.IGNORECASE):
        return True
    # key generated by a dbt macro other than config() — e.g.
    # `{{ snapshot(unique_key_col='unique_key') }}`. The macro owns the
    # projection, so it's not statically resolvable here; exclude config(),
    # which always names the key via order_by=['<col>'].
    if re.search(
        rf"\{{\{{\s*(?!config\b)\w+\([^}}]*\b{re.escape(col)}\b", text, re.IGNORECASE
    ):
        return True
    # bare or qualified select of the column by name (not the config literals,
    # which are quoted: unique_key='unique_key' / order_by=['unique_key'])
    return bool(
        re.search(rf"(?:^|[\s,]|\.)\s*{re.escape(col)}\s*,", text, re.MULTILINE)
    )
...
def audit_model(path: pathlib.Path) -> list[tuple[str, str, str]]:
    ...
    Flags the unambiguous breaking anti-pattern — a dedup-key column whose own
    projection is `Nullable(...)` / `CAST(NULL ...)` — and fails CLOSED when a
    key can be neither resolved locally nor shown to be an upstream passthrough
    (an unresolvable key is reviewed, not silently shipped). MD5 / CAST AS String
    / coalesce guards / passthroughs are fine.
    """
    ...
    for col in cols:
        ...
        projs = projections_for(text, col)
        if not projs:
            if not inherits_key(text, col):
                out.append(("ERROR", col, "unable to resolve dedup key projection"))
            continue  # passthrough / union — key audited upstream
        for proj in projs:
