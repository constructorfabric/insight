...
| `components/backend/identity-resolution/.../DESIGN.md`, `PRD.md` | Ambiguous-profile is documented as `422` with structured members; the service returns `409`. Subchart depth is documented as unbounded; it is capped and clamped. The subchart auth is documented as an `X-Insight-Person-Id` header; it is the gateway JWT. |
