...
Options:
  ...
  --host=svc1,svc2          Run selected backend services on the host instead
                            of in Compose. Recognised: analytics, identity.
...
ensure_authn_tls_certs() {
  ...
  [[ -f "$cert" ]] && openssl x509 -in "$cert" -noout -checkend 86400 2>/dev/null && openssl x509 -in "$cert" -noout -text 2>/dev/null | grep -q 'DNS:localhost' && return 0
  mkdir -p "$dir"
...
CN = authn-tls
[v3]
subjectAltName = DNS:authn-tls,DNS:localhost
EOF
...
cmd_up() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --host=*)          host_csv="${1#*=}"; shift ;;
      --host)            host_csv="$2"; shift 2 ;;
  ...
  if [[ -n "$host_csv" ]]; then
    ...
    local s
    for s in $host_csv; do
      s="$(trim "$s")"
      contains "$all_backend" "$s" || { echo "ERROR: --host only accepts analytics,identity (got: $s)" >&2; return 2; }
      host_list=$(add "$host_list" "$s")
    done
    IFS=$OLD_IFS
  fi
  ...
  for s in $host_list; do
    contains "$ghcr_list" "$s" && { echo "ERROR: $s cannot be selected by both --host and --from-ghcr/--build-only." >&2; return 2; }
  done
  ...
  local routes="deploy/compose/gateway/routes.generated.yaml"
  cp deploy/compose/gateway/routes.yaml "$routes"
  if contains "$host_list" analytics; then
    sed -i.bak "s|http://analytics:8081|http://host.docker.internal:${ANALYTICS_PORT:-8081}|" "$routes"
    rm -f "$routes.bak"
  fi
  if contains "$host_list" identity; then
    sed -i.bak "s|http://identity:8082|http://host.docker.internal:${IDENTITY_PORT:-8082}|" "$routes"
    rm -f "$routes.bak"
  fi
  mkdir -p compose
  local want_overrides=false
  [[ -n "$ghcr_list" || -n "$host_list" ]] && want_overrides=true
  {
...
YML
        ...
        if contains "$host_list" "$svc"; then
          cat <<YML
  ${svc}:
    profiles: ["host-${svc}"]
YML
...
YML
        ...
        cat <<YML
    volumes: !override
      - ${routes}:/etc/gateway/routes.yaml:ro
YML
      fi
  ...
  ensure_authn_tls_certs
  if contains "$host_list" analytics; then
    export AUTHENTICATOR_GATEWAY_ISSUER="https://localhost:${AUTHN_TLS_PORT:-8443}"
  fi
  ...
  if [[ "$skip_build" != "true" ]]; then
    ...
    if ! contains "$ghcr_list" analytics && ! contains "$host_list" analytics; then
      rust_bins="$rust_bins analytics"
    fi
    ...
    if ! contains "$ghcr_list" identity && ! contains "$host_list" identity; then
      echo "--- .NET: identity"
  ...
  if [[ -n "$host_list" ]]; then
    report_host_service_commands "$host_list"
    echo "Gateway routes selected host services through host.docker.internal."
  fi
  echo
...
report_host_service_commands() {
  local host_list="$1"
  local maria_host="${MARIADB_HOST:-mariadb}" maria_port="${MARIADB_INTERNAL_PORT:-3306}"
  ...
  for svc in $host_list; do
    case "$svc" in
      analytics)
        local host_config="deploy/compose/analytics-host.generated.yaml"
        sed -e "s|https://authn-tls:8443|https://localhost:${AUTHN_TLS_PORT:-8443}|g" -e "s|/certs/ca.pem|${ROOT_DIR}/deploy/compose/authn-tls-certs/ca.pem|g" deploy/compose/analytics-fullauth.yaml > "$host_config"
        echo "=== Host analytics ==="
        echo "cd ${ROOT_DIR}/src/backend && APP__gears__analytics__config__database_url=mysql://${MARIADB_USER:-insight}:${MARIADB_PASSWORD:-insight-local}@${maria_host}:${maria_port}/analytics APP__gears__analytics__config__clickhouse_url=http://${clickhouse_host}:${clickhouse_port} APP__gears__analytics__config__clickhouse_database=${CLICKHOUSE_DATABASE:-insight} APP__gears__analytics__config__clickhouse_user=${CLICKHOUSE_USER:-insight} APP__gears__analytics__config__clickhouse_password=${CLICKHOUSE_PASSWORD:-insight-local} APP__gears__analytics__config__identity_url=http://localhost:${IDENTITY_PORT:-8082} APP__gears__analytics__config__redis_url=redis://localhost:${REDIS_PORT:-6379} APP__gears__analytics__config__metric_catalog__tenant_default_id=${TENANT_DEFAULT_ID:-00000000-df51-5b42-9538-d2b56b7ee953} cargo run -p analytics -- -c ${ROOT_DIR}/${host_config} run"
        ;;
      identity)
        echo "=== Host identity ==="
        echo "cd ${ROOT_DIR}/src/backend && ASPNETCORE_URLS=http://0.0.0.0:${IDENTITY_PORT:-8082} IDENTITY__mariadb__url=mysql://${MARIADB_USER:-insight}:${MARIADB_PASSWORD:-insight-local}@${maria_host}:${maria_port}/identity IDENTITY__identity__bind_addr=http://0.0.0.0:${IDENTITY_PORT:-8082} IDENTITY__identity__tenant_default_id=${TENANT_DEFAULT_ID:-00000000-df51-5b42-9538-d2b56b7ee953} IDENTITY__identity__auth_gateway_issuer=${AUTHENTICATOR_GATEWAY_ISSUER:-https://authn-tls:8443} IDENTITY__identity__auth_gateway_jwks_url=http://localhost:${AUTHENTICATOR_PORT:-8083}/.well-known/jwks.json dotnet run --project services/identity/src/Insight.Identity.Api/Insight.Identity.Api.csproj"
        ;;
    esac
  done
}
