...
- The deploy hook must never be able to lose bronze rows. A heal that truncates, drops, swaps or rewrites a `bronze_*` table starts by dumping that table (`dump-clickhouse.sh`), copies into a new relation, verifies the copy against a count taken before it started, and only then swaps or drops; every step after the swap is checked explicitly and swaps the original back on failure. `run_ch` stops at the first rejected statement, guards read their counts through `ch_scalar` (a non-number stops the deploy, it is never "nothing to do"), and heals are called plainly — `f || exit 1` switches `set -e` off inside `f`. Mutations (`ALTER … UPDATE`) and `ADD COLUMN` need none of this: they cannot drop a row.
