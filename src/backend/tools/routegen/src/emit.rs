...
impl Default for Settings {
    fn default() -> Self {
        Self {
            ...
            set_real_ip_from: vec!["10.0.0.0/8".to_owned()],
...
pub fn emit(config: &RouteConfig, settings: &Settings) -> anyhow::Result<String> {
    ...
    // ── header ────────────────────────────────────────────────────────────
    c.push_str(
        "# Generated by routegen -- DO NOT EDIT.\n\
         # Source of truth: routes.yaml. Regenerate with the routegen CLI; a route\n\
         # change is a PR that re-runs it (gateway DESIGN 3.13). CI asserts this\n\
         # file is in sync with routes.yaml.\n\n",
    );
...
fn emit_http_runtime(c: &mut String, settings: &Settings, authz_url: &str) -> anyhow::Result<()> {
    ...
    // Client-IP truth: trust only the ingress hops (G8).
    c.push_str("    # --- client-IP truth: trust only the ingress hops (G8) ---\n");
...
fn emit_server(
    ...
    // Auth API + JWKS (plain proxy, no exchange -- it IS the auth).
    ...
    c.push_str("        }\n");
    c.push_str("        location = /.well-known/jwks.json {\n");
    c.push_str("            proxy_pass http://authenticator;\n");
    c.push_str("            proxy_set_header Connection \"\";\n");
    c.push_str("            proxy_set_header Host $host;\n");
    c.push_str("        }\n\n");
