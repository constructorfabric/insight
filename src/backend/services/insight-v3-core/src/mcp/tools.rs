...
pub(crate) struct RunRequest {
    ...
    /// ends after the newest row answers no rows rather than sliding back to
    /// the last day that has them. Omit it to read every row, as a run with
    /// no options always has.
