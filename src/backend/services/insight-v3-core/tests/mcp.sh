...
cleanup() {
  ...
  curl -sS -X DELETE "${BASE_URL}/v1/datasets/${DATASET}" \
    -H "Authorization: Bearer ${INSIGHT_MCP_TOKEN}" >/dev/null 2>&1 || true
}
trap cleanup EXIT
...
say "and describes what its records hold"
described="$(call describe_datasets "{\"datasets\":[\"${DATASET}\"]}")"
grep -q 'actor' <<<"${described}" || fail "the description names no field: ${described}"
...
say "a metric naming a relation of its own is refused"
refusal="$(call put_metric "{\"name\":\"${METRIC}-relational\",\"body\":{\"table\":\"events\",\"fields\":[{\"agg\":\"count\",\"type\":\"int\",\"as_name\":\"total\"}]}}")"
grep -q '"isError":true' <<<"${refusal}" || fail "a relational metric was stored: ${refusal}"
