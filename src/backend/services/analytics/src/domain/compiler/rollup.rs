...
use super::cap::{GroupCap, raw_dimension_select};
...
fn compile_capped(
    ...
    let _ = writeln!(sql, "{raw_dimensions}");
    let _ = writeln!(sql, "    FROM {}", from_clause(dataset));
    ...
    if let Some(predicate) = cap.remainder_predicate() {
    ...
    let _ = writeln!(sql, "    toUInt8(group_rank = 0) AS remainder,");
    let _ = writeln!(
        sql,
