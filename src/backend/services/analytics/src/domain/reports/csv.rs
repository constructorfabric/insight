...
mod tests {
    ...
    fn rejects_rows_that_do_not_match_planned_columns() {
        ...
        let Err(error) = writer.write_batch(&[vec![None, None]]) else {
    ...
    fn rejects_rows_that_push_generated_output_past_byte_limit() {
        ...
        writer
            .write_batch(&[vec![Some(ReportCell::Text("value".to_owned()))]])
    ...
    fn rejects_non_finite_numeric_cells() {
        ...
        let Err(error) = writer.write_batch(&[vec![Some(ReportCell::Number(f64::NAN))]]) else {
