//! Extends the metric computation vocabulary with `distinct_count`.
//!
//! `distinct_count` metrics aggregate distinct `subject_key` values with
//! `uniqExact`; like `sum` and `median` they carry no `scale`. The CHECK
//! constraint is dropped and re-added under the same name so the startup
//! CHECK probe (`REQUIRED_DEFINITION_CHECKS`) stays green, and the enum value
//! is appended at the tail so existing rows keep their ordinals.
...
const EXTEND_ENUM: &str = "ALTER TABLE metric_definitions \
     MODIFY COLUMN computation_type ENUM('sum','ratio','median','distinct_count') NOT NULL";
...
impl MigrationTrait for Migration {
    ...
    async fn down(&self, _manager: &SchemaManager) -> Result<(), DbErr> {
        Err(DbErr::Migration(
            "m20260710_000001_metric_distinct_count_computation is irreversible: \
             distinct_count definitions may exist; narrowing the enum would corrupt them."
                .to_string(),
        ))
    }
}
...
mod tests {
    ...
    /// The reconciler writes `distinct_count` only after this migration ran, so
    /// the enum text and the four-way CHECK rule must stay in lockstep with
    /// `MetricComputation::as_db` and the constraint name the startup probe
    /// asserts.
    #[test]
    fn alter_statements_pin_enum_and_check() {
        assert!(EXTEND_ENUM.contains("ENUM('sum','ratio','median','distinct_count')"));
