//! Extends the metric computation vocabulary with `percentile`.
//!
//! A percentile metric is a median at level `p/100`; the quantile `p` lives in
//! the `scale` column — the definitions table's one numeric computation
//! parameter — as an integer strictly inside (0, 100). The CHECK constraint is
//! dropped and re-added under the same name so the startup CHECK probe
//! (`REQUIRED_DEFINITION_CHECKS`) stays green, and the enum value is appended
//! at the tail so existing rows keep their ordinals.
...
const EXTEND_ENUM: &str = "ALTER TABLE metric_definitions \
     MODIFY COLUMN computation_type ENUM('sum','ratio','median','distinct_count','percentile') NOT NULL";
...
const READD_CHECK: &str = "ALTER TABLE metric_definitions \
     ADD CONSTRAINT chk_metric_definitions_computation_fields CHECK ( \
        ...
        OR (computation_type = 'percentile' AND scale IS NOT NULL AND scale > 0 AND scale < 100) \
     )";
...
impl MigrationTrait for Migration {
    ...
    async fn down(&self, _manager: &SchemaManager) -> Result<(), DbErr> {
        Err(DbErr::Migration(
            "m20260824_000001_metric_percentile_computation is irreversible: \
             percentile definitions may exist; narrowing the enum would corrupt them."
                .to_string(),
        ))
    }
}
...
mod tests {
    ...
    /// The reconciler writes `percentile` only after this migration ran, so
    /// the enum text and the five-way CHECK rule must stay in lockstep with
    /// `MetricComputation::as_db` and the constraint name the startup probe
    /// asserts.
    #[test]
    fn alter_statements_pin_enum_and_check() {
        assert!(EXTEND_ENUM.contains("ENUM('sum','ratio','median','distinct_count','percentile')"));
        ...
        assert!(READD_CHECK.contains("(computation_type = 'distinct_count' AND scale IS NULL)"));
        assert!(READD_CHECK.contains(
            "(computation_type = 'percentile' AND scale IS NOT NULL AND scale > 0 AND scale < 100)"
        ));
    }
}
