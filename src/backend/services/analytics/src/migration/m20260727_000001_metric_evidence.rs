...
async fn replace_evidence_constraints(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    ...
    for (name, ddl) in constraints {
        if check_constraint_exists(manager, "metric_sources", name).await? {
            conn.execute_unprepared(&format!("ALTER TABLE metric_sources DROP CHECK {name}"))
                .await?;
        }
        conn.execute_unprepared(ddl).await?;
    }
...
async fn check_constraint_exists(
    ...
    let row = manager
        ...
        .query_one(Statement::from_sql_and_values(
            ...
            [table.into(), constraint.into()],
        ))
        .await?;
    Ok(row
        .and_then(|row| row.try_get::<u64>("", "constraint_count").ok())
        .unwrap_or_default()
        > 0)
}
