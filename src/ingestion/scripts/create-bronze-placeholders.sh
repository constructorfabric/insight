...
#
# The snapshot is generated by running the full bootstrap-db pipeline — real
# connector `discover`, the real destination-clickhouse connector, real dbt
# models — against a throwaway ClickHouse and dumping SHOW CREATE for everything
# it produced (scripts/bootstrap-db/dump-ddl.sh), then committing the result.
# .github/workflows/connectors-ddl.yml re-runs that pipeline on every PR and
# fails on any drift; it validates, it never commits. The schemas
