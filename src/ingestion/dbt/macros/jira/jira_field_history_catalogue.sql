{#
  ...
  rebuilt in full. `epoch_ms` is the catalogue's own extraction stamp at that
  rebuild; every row's `_version` is at least that, so a rebuild reaches the
  incremental class even where an issue's own inputs did not move
  (FIELD-HISTORY-IN-DBT.md §7).
#}
...
{#- {rebuild, fingerprint, epoch_ms}.

    Pure in the catalogue and the table's comment, so the model body and the
    post-hook of one run agree on the epoch — including across a rebuild, which
    swaps in a table carrying no comment and therefore reads as a rebuild from
    the hook as well.

    `flags.FULL_REFRESH`, not `should_full_refresh()`: the model sets the
    `full_refresh` CONFIG to carry a rebuild out, and reading the config back
    here would make every later call see a rebuild the operator never asked
    for. -#}
{% macro jira_journal_catalogue_state() %}
    {%- if not execute -%}
        {{ return({'rebuild': true, 'fingerprint': '', 'epoch_ms': 0}) }}
    {%- endif -%}
...
{#- post_hook. Runs only after the build succeeded, so a failed run leaves the
    previous record in place and the next run rebuilds again. A run that could
    not read the catalogue records nothing, which reads as "unknown" and
    rebuilds next time. -#}
{% macro jira_journal_record_catalogue() %}
    {%- set state = jira_journal_catalogue_state() -%}
    {%- if execute and state.fingerprint != '' -%}
        ALTER TABLE {{ this }} MODIFY COMMENT '{{ jira_journal_catalogue_marker() }} fingerprint={{ state.fingerprint }} epoch_ms={{ state.epoch_ms }}'
    {%- else -%}
        SELECT 1
    {%- endif -%}
{% endmacro %}
