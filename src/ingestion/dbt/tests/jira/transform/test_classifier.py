...
def test_an_unclassifiable_field_stops_the_run(scenario: Scenario) -> None:
    ...
    attempt = scenario.warehouse.dbt_status("run", "--select", "tag:jira,tag:staging")

    assert not attempt.success, "an unmapped field kind must fail the run"
    # ClickHouse requires throwIf's message to be constant, so it names where to
    # look rather than which field.
    assert "jira__task_field_kind" in attempt.log
