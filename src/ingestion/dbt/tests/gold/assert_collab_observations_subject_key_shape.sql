...
-- distinct-count measures (active_day, active_tool) and absent everywhere
...
HAVING (measure_key IN ('active_day', 'active_tool') AND null_subject_rows > 0)
    OR (measure_key NOT IN ('active_day', 'active_tool') AND subject_rows > 0)
