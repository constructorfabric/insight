{{ config(
    ...
    meta={
        ...
        'category': 'coverage',
        'tier': 'error',
        'remediation': 'A row here is an invoice whose Stripe hosted chain did not complete, so its money is on the ledger with no per-seat price behind it and ai.seat_cost is missing that tier for that month. chain_status says which hop failed: unparsable_url means the hosted_invoice_url did not match the expected form, which for more than an odd invoice means the URL format changed and the connector needs updating; failed means a hop answered badly — check egress to invoicedata.stripe.com and api.stripe.com, and that the pinned Stripe-Version is still accepted. Re-running the connector re-follows a freshly issued URL, so a transient failure clears itself on the next sync.'
    }
) }}
