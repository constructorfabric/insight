{{ config(
    ...
    meta={
        ...
        'category': 'coverage',
        'tier': 'error',
        'remediation': 'A row here is a connector instance whose subscription stream produced rows before and none in the last three days. The connector maps 401, 403 and 404 on /api/subscriptions/{org_id}/usage to action: IGNORE, so every way of losing billing leaves the sync green and the stream empty. The connector''s log stream distinguishes them and is the only place that does — on action: IGNORE the CDK writes the filter''s message with logger.info and nothing structured survives, so this is a log line to read, not a field to query: 401 means the workspace role fell below account-admin and this installation now has no billing capability at all; 403 means the session was gated out of billing; 404 means chatgpt_org_id is blank or wrong, or the proxy route is broken. Fix the one the log names, then re-sync. An instance deliberately decommissioned is the other explanation, and it clears itself once its last reporting day rolls out of the comparison. Note the blind spot this check cannot cover: an installation that has NEVER been able to read billing produces no row on either side of the comparison, so its only signal is the 401 logged on its first run.'
    }
) }}
