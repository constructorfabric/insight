...
-- Authored file changes: one row per path a commit touched.

WITH
-- INVARIANT: one row per change CONTENT, not per commit carrying it, earliest
-- commit winning — summing both commits' diffs would count those lines twice.
--
-- SAFETY: the commit_hash tie-breaker keeps rows with no oid distinct per
-- commit, which LIMIT 1 BY would otherwise collapse on their equal NULL keys.
