...
def get_login_id_pairs(roster: list[Person]) -> list[tuple[str, str]]:
    ...
    `value_type='id'` login-bootstrap observations.

    `keycloak_realm` sets EVERY realm user's Keycloak `id` to that person's
    OWN roster UUID (`"id": person.uuid`), and Keycloak issues `sub` equal to
    the user's internal id verbatim — so every seeded persona's external id
    IS their own roster uuid, and the whole roster can log in (matching the
    realm, which seeds all of them). Getting this wrong means the
    login-bootstrap 403s: the seeded `value_type='id'` row would carry a
    value the id_token never presents.
    """
...
def get_idp_source_type() -> str:
    ...
    lookup finds."""
    return os.environ.get("IDP_SOURCE_TYPE", "keycloak").strip() or "keycloak"
