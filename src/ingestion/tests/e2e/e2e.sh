...
# SSOT) via the compose/docker-compose.e2e.yml overlay, under an ISOLATED `insight-e2e`
# compose project (own volumes/containers/network — a running `./dev-compose.sh
# up` dev stack is left untouched).
#
# Three phases (see compose/docker-compose.e2e.yml):
...
PROJECT=insight-e2e
# Base clickhouse + mariadb defs = the ROOT compose (shared with the dev stack,
# single version SSOT); the e2e overlay lives next to Dockerfile.runner in this
# dir's compose/. --project-directory (in dc() below) pins build-context
# resolution to the repo root regardless of where these files sit.
E2E_COMPOSE_DIR="$(pwd)/compose"
COMPOSE_FILES=(-f "$INSIGHT_REPO_ROOT/docker-compose.yml" -f "$E2E_COMPOSE_DIR/docker-compose.e2e.yml")

# Optional extra overlays (space-separated), resolved relative to compose/.
...
if [ -n "${E2E_COMPOSE_OVERLAYS:-}" ]; then
    for overlay in ${E2E_COMPOSE_OVERLAYS}; do
        COMPOSE_FILES+=(-f "$E2E_COMPOSE_DIR/$overlay")
    done
fi
