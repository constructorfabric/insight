...
def test_every_decided_run_counts_once_whatever_the_trigger(spec: SpecRun) -> None:
    ...
    assert any(some(series["points"], bucket_start="2026-03-01", value=3.0) for series in r.series("ci.runs"))
    assert some(r.breakdown("ci.runs"), dimensions={"key": "trigger", "value": "schedule"}, value=1.0)
