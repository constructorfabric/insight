...
def test_pr_comments_split_into_own_and_others_and_the_halves_add_up(spec: SpecRun) -> None:
    ...
    r.row("git.pr_comments", "peer", entity_id=ALICE).equals(target_value=3, p25=1, median=1, p75=1, min=1, max=3, n=5)
    r.row("git.pr_comments", "timeseries", entity_id=ALICE).contains(points={"bucket_start": "2026-10-01", "value": 3})
    r.row("git.pr_comments", "breakdown", entity_id=ALICE, dimensions={"key": "comment_target", "value": "own"}).equals(
        value=2
    )
    r.row(
        "git.pr_comments", "breakdown", entity_id=ALICE, dimensions={"key": "comment_target", "value": "others"}
    ).equals(value=1)
    ...
    r = spec.call(
        {
            ...
            "body": {
                ...
                "period": {"from": "2026-10-01", "to": "2026-10-02"},
                "metrics": [
                    {"metric_key": "git.pr_comments", "views": [{"view": "breakdown", "dimensions": ["source"]}]}
                ],
            },
        }
    )
    assert r.status == 200
    r.row("git.pr_comments", "breakdown", entity_id=ALICE, dimensions={"key": "source", "value": "github"}).equals(
        value=3
    )
...
def test_comment_on_a_request_whose_author_is_unknown_counts_under_others(spec: SpecRun) -> None:
    ...
    r = spec.call(
        {
            ...
            "body": {
                ...
                "metrics": [
                    {
                        "metric_key": "git.pr_comments",
                        "views": [{"view": "period"}, {"view": "breakdown", "dimensions": ["comment_target"]}],
                    }
                ],
            },
        }
    ...
    r.row(
        "git.pr_comments", "breakdown", entity_id=DAVE, dimensions={"key": "comment_target", "value": "others"}
    ).equals(value=1)
