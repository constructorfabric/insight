...
def test_duration_spread_is_the_sample_stddev_over_runs(spec: SpecRun) -> None:
    ...
    march_second = some(
        [p for s in r.series("ci.run_duration_min_stddev") for p in s["points"]], bucket_start="2026-03-02"
    )
    assert any(2.1 < float(p["value"]) < 2.2 for p in march_second if p["value"] is not None), march_second

    by_repository = some(
        r.breakdown("ci.run_duration_min_stddev"), dimensions={"key": "repository", "value": "acme/app"}
    )
    assert any(2.1 < float(v["value"]) < 2.2 for v in by_repository if v["value"] is not None), by_repository
    ...
    r = spec.call(
        {
            ...
            "body": {
                ...
                "metrics": [{"metric_key": "ci.run_duration_min_stddev", "views": [{"view": "period"}]}],
            },
        }
    )
    assert r.status == 200
