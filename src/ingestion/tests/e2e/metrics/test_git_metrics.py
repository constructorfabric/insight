...
def test_repository_rollups_pool_observations_across_people(spec: SpecRun) -> None:
    ...
    r = spec.call(
        {
            ...
            "body": {
                ...
                "period": {"from": "2026-10-01", "to": "2026-10-02"},
                "metrics": [
                    {"metric_key": "git.merge_rate", "views": [{"view": "rollup", "dimensions": ["repository"]}]},
                    {"metric_key": "git.pr_cycle_time_h", "views": [{"view": "rollup", "dimensions": ["repository"]}]},
                ],
            },
        }
    )
    ...
    r.row("git.pr_cycle_time_h", "rollup", dimensions=REPOSITORY_INSIGHT).equals(value=3, contributing_entity_count=5)
...
def test_unified_git_metrics(spec: SpecRun) -> None:
    ...
    r.row("git.commits", "peer", entity_id=ERIN).equals(target_value=2, p25=1, median=1, p75=1, min=1, max=2, n=5)
    r.row("git.commits", "timeseries", entity_id=ERIN).contains(points={"bucket_start": "2026-10-01", "value": 2})
    ...
    r.row("git.code_lines", "timeseries", entity_id=ERIN).contains(points={"bucket_start": "2026-10-01", "value": 50})
    ...
    r.row("git.lines_added", "timeseries", entity_id=ERIN).contains(points={"bucket_start": "2026-10-01", "value": 100})
    ...
    r.row("git.lines_removed", "breakdown", entity_id=ERIN, dimensions=CATEGORY_CODE).equals(value=10)

    r.row("git.prs_created", "period", entity_id=ERIN).equals(value=1)
    r.row("git.prs_created", "peer", entity_id=ERIN).equals(target_value=1, p25=1, median=1, p75=1, min=1, max=1, n=5)
    r.row("git.prs_created", "timeseries", entity_id=ERIN).contains(points={"bucket_start": "2026-10-01", "value": 1})
    ...
    r.row("git.prs_merged", "peer", entity_id=ERIN).equals(target_value=1, p25=1, median=1, p75=1, min=1, max=1, n=5)
    r.row("git.prs_merged", "timeseries", entity_id=ERIN).contains(points={"bucket_start": "2026-10-01", "value": 1})
    ...
    r.row("git.merge_rate", "timeseries", entity_id=ERIN).contains(points={"bucket_start": "2026-10-01", "value": 100})
    ...
    assert some(commits_per_repository, dimensions={"key": "repository"}, value=1.0) == commits_per_repository
    ...
    r.row("git.commit_size", "timeseries", entity_id=ERIN).contains(points={"bucket_start": "2026-10-01", "value": 60})
    ...
    r.row("git.pr_size", "peer", entity_id=ERIN).equals(target_value=50, p25=20, median=30, p75=40, min=10, max=50, n=5)
    r.row("git.pr_size", "timeseries", entity_id=ERIN).contains(points={"bucket_start": "2026-10-01", "value": 50})
    ...
    r.row("git.pr_cycle_time_h", "breakdown", entity_id=ERIN, dimensions=SOURCE_GITHUB).equals(value=5)
    ...
    r.row("git.test_change_share", "breakdown", entity_id=ERIN, dimensions=SOURCE_GITHUB).equals(value=0)
    ...
    r.row("git.pr_abandonment_rate", "breakdown", entity_id=ERIN, dimensions=SOURCE_GITHUB).equals(value=0)
    ...
    r.row("git.review_coverage", "breakdown", entity_id=ERIN, dimensions=SOURCE_GITHUB).equals(value=100)
    ...
    r.row("git.reviewers_per_pr", "breakdown", entity_id=ERIN, dimensions=SOURCE_GITHUB).equals(value=2)
    ...
    r.row("git.multi_reviewer_rate", "breakdown", entity_id=ERIN, dimensions=SOURCE_GITHUB).equals(value=100)
    ...
    r.row("git.merges_without_approval_rate", "breakdown", entity_id=ERIN, dimensions=SOURCE_GITHUB).equals(value=0)

    r.row("git.active_days", "period", entity_id=ERIN).equals(value=1)
    r.row("git.active_days", "peer", entity_id=ERIN).equals(target_value=1, p25=1, median=1, p75=1, min=1, max=1, n=5)
    r.row("git.active_days", "timeseries", entity_id=ERIN).contains(points={"bucket_start": "2026-10-01", "value": 1})
    active_days_per_repository = r.breakdown("git.active_days")
    assert len(active_days_per_repository) == 2
    assert some(active_days_per_repository, dimensions={"key": "repository"}, value=1.0) == active_days_per_repository
    ...
    r.row("git.first_review_time_h", "breakdown", entity_id=ERIN, dimensions=SOURCE_GITHUB).equals(value=2.5)
    ...
    r.row("git.review_wait_share", "breakdown", entity_id=ERIN, dimensions=SOURCE_GITHUB).equals(value=50)
    ...
    r.row("git.review_to_merge_time_h", "breakdown", entity_id=ERIN, dimensions=SOURCE_GITHUB).equals(value=2.5)
    ...
    r.row("git.approval_to_merge_time_h", "breakdown", entity_id=ERIN, dimensions=SOURCE_GITHUB).equals(value=2.5)
