...
def test_tasks_dev_time(spec: SpecRun) -> None:
    ...
    r.row("tasks.dev_time", "peer", entity_id=ERIN).equals(target_value=5, p25=2, median=3, p75=4, min=1, max=5, n=5)
    ...
    by_source = one(r.breakdown("tasks.dev_time"), entity_id=ERIN, dimensions={"key": "source", "value": "jira"})
    ...
    r = spec.call(
        {
            ...
            "body": {
                "entity": {"type": "person", "ids": [ERIN]},
                "period": {"from": "2026-03-20", "to": "2026-03-31"},
                "metrics": [{"metric_key": "tasks.dev_time", "views": [{"view": "breakdown", "dimensions": ["type"]}]}],
            },
        }
    )
    assert r.status == 200

    by_type = one(r.breakdown("tasks.dev_time"), entity_id=ERIN, dimensions={"key": "type", "value": "Task"})
