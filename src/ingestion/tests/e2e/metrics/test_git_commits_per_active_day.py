...
def test_commits_divide_by_distinct_active_days(spec: SpecRun) -> None:
    ...
    r.row("git.commits_per_active_day", "breakdown", entity_id=ERIN, dimensions=API).equals(value=1.5)
    ...
    r = spec.call(
        {
            ...
            "body": {
                ...
                "metrics": [{"metric_key": "git.commits_per_active_day", "views": [{"view": "period"}]}],
            },
        }
    )
    assert r.status == 200
