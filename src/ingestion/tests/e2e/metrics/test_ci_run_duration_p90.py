...
def test_p90_duration_reads_the_tail_of_the_run_distribution(spec: SpecRun) -> None:
    ...
    day = some([p for s in r.series("ci.run_duration_min_p90") for p in s["points"]], bucket_start="2026-03-02")
    assert any(8.0 <= float(p["value"]) <= 9.0 for p in day if p["value"] is not None)

    by_pipeline = some(
        r.breakdown("ci.run_duration_min_p90"), dimensions={"key": "pipeline", "value": ".github/workflows/ci.yml"}
    )
