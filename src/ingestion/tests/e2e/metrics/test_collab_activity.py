...
def test_unified_collaboration_activity(spec: SpecRun) -> None:
    ...
    by_tool = one(r.breakdown("collab.active_days"), entity_id=ERIN, dimensions={"key": "tool", "value": "m365"})
    assert float(by_tool["value"]) == 5.0

    r.row("collab.breadth", "period", entity_id=ERIN).equals(value=1)
    r.row("collab.breadth", "peer", entity_id=ERIN).equals(target_value=1, p25=1, median=1, p75=1, min=1, max=1, n=5)
