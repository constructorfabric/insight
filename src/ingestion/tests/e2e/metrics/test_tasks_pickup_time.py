...
def test_tasks_pickup_time(spec: SpecRun) -> None:
    ...
    r.row("tasks.pickup_time", "peer", entity_id=ERIN).equals(target_value=5, p25=2, median=3, p75=4, min=1, max=5, n=5)
    ...
    by_source = one(r.breakdown("tasks.pickup_time"), entity_id=ERIN, dimensions={"key": "source", "value": "jira"})
...
def test_tasks_pickup_time_by_issue_type(spec: SpecRun) -> None:
    ...
    r = spec.call(
        {
            ...
            "body": {
                ...
                "period": {"from": "2026-03-20", "to": "2026-03-31"},
                "metrics": [
                    {"metric_key": "tasks.pickup_time", "views": [{"view": "breakdown", "dimensions": ["type"]}]}
                ],
            },
        }
    )
    ...
    by_type = one(r.breakdown("tasks.pickup_time"), entity_id=ERIN, dimensions={"key": "type", "value": "Task"})
