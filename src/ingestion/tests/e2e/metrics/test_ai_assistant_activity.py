...
def test_ai_assistant_actions_and_chat_conversations(spec: SpecRun) -> None:
    ...
    claude_actions = some(
        r.breakdown("ai.assistant_actions"), entity_id=ERIN, dimensions={"key": "tool", "value": "claude"}
    )
    ...
    claude_conversations = some(
        r.breakdown("ai.chat_assistant_conversations"), entity_id=ERIN, dimensions={"key": "tool", "value": "claude"}
    )
