...
def test_tasks_closed(spec: SpecRun) -> None:
    ...
    assert float(one(by_type, entity_id=ERIN, dimensions={"key": "type", "value": "Task"})["value"]) == 3.0
    assert float(one(by_type, entity_id=ERIN, dimensions={"key": "type", "value": "Bug"})["value"]) == 1.0
    assert float(one(by_type, entity_id=ERIN, dimensions={"key": "type", "value": "Incident"})["value"]) == 1.0
