...
def test_ai_daily_extra_usage_never_reports_a_negative_day(spec: SpecRun) -> None:
    ...
    for entry in series:
        ...
        assert all(day >= 0.0 for day in spent), f"should never report a negative day: {entry['entity_id']} {spent!r}"
    ...
    r = spec.call(
        {
            ...
            "body": {
                ...
                "period": {"from": "2026-11-01", "to": "2026-12-12"},
                "metrics": [
                    {"metric_key": "ai.daily_approximate_extra_usage_cost", "views": [{"view": "period"}]},
                    {"metric_key": "ai.extra_usage_cost", "views": [{"view": "period"}]},
                ],
            },
        }
    ...
    r = spec.call(
        {
            ...
            "body": {
                ...
                "metrics": [{"metric_key": "ai.daily_approximate_extra_usage_cost", "views": [{"view": "period"}]}],
            },
        }
    )
    assert r.status == 200
