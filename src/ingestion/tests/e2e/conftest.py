...
compose/docker-compose.e2e.yml). pytest connects to that already-migrated stack; it does
NOT boot compose or apply base migrations. It then:
  ...
  session build (once, `build_world` fixture — see lib/seed_once.py):
    ...
    6. (analytics fixture) spawn analytics + seed metric definitions
...
(not xdist-safe: build_world and the shared analytics process are
single-owner).
"""
...
# analytics process are single-owner). We still elect a primary worker so a
...
def stack_ready(session_cfg: SessionConfig) -> SessionConfig:
    ...
    pytest starts. This fixture does NOT boot or migrate anything.

    No ClickHouse readiness probe is needed: `e2e-migrate` runs the real
    apply-ch-migrations.sh and BLOCKS until it succeeds before e2e-runner (pytest)
    starts, so by the time we get here CH has already served dozens of real
    migration queries — a retry loop could never retry. MariaDB still gets a probe:
    it is not migrated pre-pytest, and the container can report healthy a beat
    before `SELECT 1` succeeds (init double-start), which `up --wait` does not fully
    close. `wait_ready` returns immediately once the DB actually answers.
    """
    if _IS_PRIMARY:
...
@pytest.fixture(scope="session")
def analytics(build_world: SessionConfig):
    """Spawn the analytics binary baked into the runner image, AFTER the
