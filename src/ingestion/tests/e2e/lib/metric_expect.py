...
def matches(value: Any, selector: Any) -> bool:
    if isinstance(selector, dict):
        if isinstance(value, dict):
            return all(key in value and matches(value[key], expected) for key, expected in selector.items())
...
class Ledger:
    ...
    def write(self, path: Path) -> None:
        document = {
            "asserted": {
                key: {view: sorted(names) for view, names in views.items()} for key, views in self.asserted.items()
            },
...
class MetricResponse:
    ...
    def metric(self, key: str) -> dict[str, Any]:
        ...
        if len(found) != 1:
            raise ExpectError(f"{self._test_name}: metric {key!r} matched {len(found)} metrics (expected exactly 1)")
        return found[0]

    def view(self, key: str, kind: str) -> dict[str, Any]:
        found = [view for view in self.metric(key).get("views", []) if view.get("view") == kind]
        if len(found) != 1:
            raise ExpectError(f"{self._test_name}: {key} view {kind!r} matched {len(found)} views (expected exactly 1)")
        return found[0]
    ...
    def check_complete(self) -> None:
        ...
        for (key, kind, _), row in self._touched.items():
            ...
            if missing:
                raise ExpectError(f"{self._test_name}: {key}/{kind} row leaves {sorted(missing)} unasserted")
