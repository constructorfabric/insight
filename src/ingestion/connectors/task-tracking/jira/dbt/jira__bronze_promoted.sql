...
{# The four Jira catalogues are full-refresh streams: every sync re-appends
   the whole set, so a plain MergeTree grows by one copy per sync and `FINAL`
   on it raises `Storage MergeTree doesn't support FINAL` (#1886). They were
   absent from this list, so they never left MergeTree.
   jira_boards is NOT promoted yet: its inline schema only just gained the
   AddFields columns, so existing bronze rows carry a NULL unique_key — and
   RMT treats NULL keys as equal, which would collapse the whole table into
   one row. Promote it after a sync with the fixed schema has populated the
   key. #}
