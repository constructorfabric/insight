...
- **Date-window boundary**: the usage/cost API treats `ending_at` as **exclusive** and snaps `starting_at` to the UTC day start, so a `midnight → next-midnight` window selects exactly one daily bucket. `claude_admin_messages_usage` and `claude_admin_cost_report` use `cursor_granularity: P0D` (`ending_at = starting_at + P1D`, the next midnight); the earlier `PT1S` sent `23:59:59Z`, which is not before the bucket's midnight end, so `cost_report` 400'd with `Invalid date range` and ingested 0 rows. `claude_admin_code_usage` keeps `PT1S` because it sends only `starting_at`. See [ADR-0002](../../../../../docs/components/connectors/ai/claude-admin/specs/ADR/0002-cost-report-exclusive-ending-boundary.md) (supersedes ADR-0001).
