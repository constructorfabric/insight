 ...
 * the rest.
 *
 * Where the variants ask for the same names but read them differently -
 * a metric's fields name a dataset's declared field or a table's column,
 * never both - the shape says `resets` and the record starts over. Carrying
 * a value written for the other source across would leave a body that looks
 * filled in and cannot be sent; starting over is one click to undo, and
 * leaving without saving still restores what was stored.
 */
export function switched(
  ...
  const kept = shape.resets ? {} : carried(shape, record, to);
