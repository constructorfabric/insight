...
class TestYaml:
    ...
    bronze: dict[str, list[dict[str, Any]]] = field(default_factory=dict)
...
def load(
    ...
    *,
    ...
    if not isinstance(aliases_doc, dict):
        raise FixtureError(
            f"{path}: `identity_aliases` must be a mapping of canonical email → aliases"
        )
...
def _with_derived_payload(record: dict[str, Any], schema: dict[str, Any]) -> dict[str, Any]:
