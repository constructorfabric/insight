...
view itself renders behind `AdminGate`. The server refuses the summary regardless
of what the frontend draws, which is what `tests/stand/api` covers — here the
interest is what an admin SEES.

Measured on a stand serving the SPA from `src/frontend` at this commit:
...
real data: `tr[data-slot="table-row"]` for every row, `th[data-slot="table-head"]`
...
container, which `flows._scrolled` does.

**The Page column shows the screen's label, not its path.** `screenLabel()` turns
`/portal/manage/platform-usage` into the zone and item names joined by its own
`SEPARATOR`; the raw path is in a tooltip that renders on hover, as
`[data-slot="tooltip-content"]`. So a journey asserting what was recorded hovers
the cell.
"""
