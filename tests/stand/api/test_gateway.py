...
def test_operation_is_refused_without_a_session(
    ...
    enough to do once per catalogued operation — `ProblemDocument` also forbids undeclared fields, so
...
def test_the_refusal_names_the_way_back_in(api_client: ApiClient) -> None:
    ...
    for everything it fronts, so one copy per operation would restate one fact. What the sweep
