...
def test_env_kind_splits_production_transient_preview_and_static(spec: SpecRun) -> None:
    ...
    r = spec.call(
        {
            ...
            "body": {
                ...
                "metrics": [
                    {
                        "metric_key": "ci.deployments",
                        "views": [{"view": "breakdown", "dimensions": ["env_kind"]}],
                    }
                ],
            },
        }
    ...
    assert (
        float(one(by_env_kind, dimensions={"key": "env_kind", "value": "production"})["value"])
        == 1.0
    )
    assert (
        float(one(by_env_kind, dimensions={"key": "env_kind", "value": "preview"})["value"]) == 1.0
    )
    assert (
        float(one(by_env_kind, dimensions={"key": "env_kind", "value": "static"})["value"]) == 1.0
    )
