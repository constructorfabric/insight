...
import pytest
from insight_datapath.metric_expect import MetricResponse, one
...
def erin_value_for_tool(r: MetricResponse, key: str, tool: str) -> float:
    return float(
        one(r.breakdown(key), entity_id=ERIN, dimensions={"key": "tool", "value": tool})["value"]
    )
...
def test_unified_collaboration_messaging(spec: SpecRun) -> None:
    ...
    assert (
        one(
            one(r.series("collab.channel_posts"), entity_id=ERIN)["points"],
            bucket_start="2026-12-25",
        )["value"]
        is None
    )
