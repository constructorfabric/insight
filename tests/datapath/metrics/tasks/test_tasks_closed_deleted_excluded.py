...
events in the field history, and gold excludes only deleted/trashed issues. Dave's two
bugs — one alive, one deleted — show the exclusion reaching the bug subset too.
"""
...
def test_deleted_issue_leaves_tasks_closed_access_lost_issue_stays(spec: SpecRun) -> None:
    ...
    the department distribution {1,2,4,5,5} follows the same rule."""
    r = spec.call(
        {
            ...
            "body": {
                ...
                "metrics": [
                    {"metric_key": "tasks.closed", "views": [{"view": "period"}, {"view": "peer"}]}
                ],
            },
        }
    )
    ...
    r.row("tasks.closed", "peer", entity_id=ERIN).equals(
        target_value=5, p25=2, median=4, p75=5, min=1, max=5, n=5
    )


def test_deleted_bug_leaves_bugs_fixed(spec: SpecRun) -> None:
    """Dave closes two bugs, but BUGX-1 is deleted at the source: bugs_fixed is 1, and
    the surviving close lifts his tasks.closed to 5."""
    r = spec.call(
        {
            "url": "/v1/metric-results",
            "method": "POST",
    ...
    assert r.status == 200
    r.row("tasks.bugs_fixed", "period", entity_id=DAVE).equals(value=1)
    r.row("tasks.closed", "period", entity_id=DAVE).equals(value=5)
