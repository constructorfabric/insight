"""Jira's bug split comes from the operator's value map, keyed by issue-type id.

There is no name-list fallback: an id with no live row is `unknown`. Carol closes a
type named Bug whose live row says `other` (non-bug), a custom type whose live row
...
def test_live_map_rows_classify_by_id_and_anything_else_is_unknown(spec: SpecRun) -> None:
