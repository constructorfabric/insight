...
translated name, literal Bug, each decided `bug` by id) and (5-r) tasks, so bugs_fixed
...
def test_decided_types_classify_as_bug(spec: SpecRun, email: str, bugs_fixed: int) -> None:
    """Each member's bugs carry a distinct type; a decided id counts as a bug however
