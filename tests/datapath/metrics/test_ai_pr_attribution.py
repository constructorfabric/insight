...
def test_ai_pull_request_attribution(spec: SpecRun) -> None:
    ...
    assert float(one(with_assistant, bucket_start="2026-01-05")["value"]) == approx(6.0)
    by_seat_status = one(
        r.breakdown("ai.prs_with_assistant"),
        entity_id=ALICE,
        dimensions={"key": "seat_status", "value": "active"},
    )
    assert float(by_seat_status["value"]) == approx(6.0)
    ...
    assert float(one(total, bucket_start="2026-01-05")["value"]) == approx(10.0)
    by_tool = one(
        r.breakdown("ai.prs_total"),
        entity_id=ALICE,
        dimensions={"key": "tool", "value": "claude_code"},
    )
    assert float(by_tool["value"]) == approx(10.0)
