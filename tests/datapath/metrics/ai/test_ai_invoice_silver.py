...
def _read_class(cfg: InstanceConfig, source_id: str, order_by: str = "unique_key") -> list[dict]:
...
def test_every_bronze_row_reaches_the_class(invoice_silver: list[dict]) -> None:
    assert len(invoice_silver) == len(BRONZE_ROWS)


def test_each_tier_keeps_its_own_seat_price(invoice_silver: list[dict]) -> None:
...
def test_a_proration_keeps_its_money_and_prices_no_seat(invoice_silver: list[dict]) -> None:
...
def test_extra_usage_is_overusage_and_prices_no_seat(invoice_silver: list[dict]) -> None:
...
def test_an_invoices_money_sits_on_its_own_row_alone(invoice_silver: list[dict]) -> None:
    ...
    assert sum(r["invoice_net_cents"] for r in invoices) == 18_249, (
        "16000 - 1500 + 2000 + 999 + 750"
    )


def test_a_failed_chain_keeps_the_invoice_and_no_line(invoice_silver: list[dict]) -> None:
...
def test_an_invoice_with_no_hosted_url_reaches_the_class_as_its_own_state(
    invoice_silver: list[dict],
) -> None:
...
def test_a_row_is_dated_by_the_period_it_charges_for(invoice_silver: list[dict]) -> None:
    ...
    monthly = next(
        r for r in invoice_silver if r["invoice_id"] == "in_MONTHLY" and not r["line_id"]
    )
    assert monthly["period_month"] == "2026-08-01", "the invoice's money is filed with its lines"


def test_an_invoice_with_no_line_falls_back_to_the_day_it_was_raised(
    invoice_silver: list[dict],
) -> None:
...
def test_a_recovery_inside_one_build_leaves_no_gap_behind(
    recovered_in_one_build: list[dict],
) -> None:
...
def test_a_recovery_across_two_builds_leaves_no_gap_behind(
    recovered_across_two_builds: list[dict],
) -> None:
...
def test_a_second_source_instance_is_not_shut_out_by_the_first(
    second_instance_silver: list[dict],
) -> None:
